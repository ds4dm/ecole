...
template <typename T> struct is_environment<T, void_t<decltype(&T::step)>> : std::true_type {};
...
template <typename T> struct observation_of<T, std::enable_if_t<is_environment<T>::value>> {
...
template <typename T> struct action_set_of<T, std::enable_if_t<is_environment<T>::value>> {
