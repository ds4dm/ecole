...
auto is_lp_event(SCIP_EVENTTYPE event) {
	...
	return ((event == SCIP_EVENTTYPE_LPEVENT) | (event == SCIP_EVENTTYPE_FIRSTLPSOLVED) |
					(event == SCIP_EVENTTYPE_LPSOLVED)) != 0;
}
...
template <Bound bound> BoundIntegral<bound>::BoundIntegral(bool wall_, const BoundFunction& bound_function_) {
	wall = wall_;
	if (bound_function_) {
...
template <Bound bound> void BoundIntegral<bound>::before_reset(scip::Model& model) {
	...
	if constexpr (bound == Bound::dual) {
		...
		SCIPincludeObjEventhdlr(
			model.get_scip_ptr(), new IntegralEventHandler(model.get_scip_ptr(), wall, TRUE, TRUE), TRUE);
	}
